"""
StorageClient -- typed proxy for storage_service over HTTP.

Uploads are performed through presigned MinIO URLs so file bytes do not
travel through internal RPC payloads.
"""
...
class _StorageProxy:
    ...
    async def delete(self, path: str) -> bool:
        ...
        result = await self._call("delete", path=normalized_path)
...
class StorageClient:
    ...
    def __init__(self):
        ...
        base_url = os.getenv("STORAGE_SERVICE_HTTP_URL", "http://127.0.0.1:8004").rstrip("/")
        ...
        self.storage = _StorageProxy(self._http, "storage")
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            logger.info("StorageClient connected to storage_service via HTTP.")
        except Exception as exc:
            logger.error("Failed to connect StorageClient: %s", exc, exc_info=True)
            self._connected = False
            raise
    ...
    async def disconnect(self) -> None:
        if self._connected:
            ...
            self._connected = False
            logger.info("StorageClient disconnected.")
    ...
    def get_storage_url(self, path: str) -> str:
        ...
        if base:
            return f"{base}/storage/{normalized_path}"
        return f"{self._http.base_url}/storage/{normalized_path}"
